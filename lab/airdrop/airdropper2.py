...
def install_and_import(package_name, import_name=None):
    """
    Versucht das Modul zu importieren und installiert es bei ImportError automatisch.
    :param package_name: Name des Pakets, wie es mit pip installiert wird.
    :param import_name: Name des Moduls, falls dieser von package_name abweicht.
    """
    try:
        if import_name:
        ...
        try:
            ...
            sys.exit(1)
        # Nach erfolgreicher Installation nochmal importieren
        if import_name:
            globals()[import_name] = __import__(import_name)
        else:
            globals()[package_name] = __import__(package_name)

# Automatische Installation benötigter Pakete
