...
def install_if_missing(package, import_name=None):
    """
    Versucht, das Modul per __import__ zu laden; bei ImportError wird das Package automatisch installiert.
    Dabei kann import_name angegeben werden, falls der Modulname vom Package-Namen abweicht.
    """
    module_name = import_name if import_name else package
    try:
        __import__(module_name)
    except ImportError:
        print(f"Package '{package}' wird installiert ...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", package])

# Benötigte Pakete prüfen und ggf. installieren
install_if_missing("requests")
install_if_missing("solana")
# Das Paket wird als "spl-token" installiert, der Import erfolgt aber aus dem Modul "spl.token"
install_if_missing("spl-token", "spl.token")

# Nachdem alle Pakete installiert sind, importieren wir die nötigen Module
import json
import logging
import time
import requests
...
def send_token_transaction(client: Client, sender: Keypair, recipient_address: str, amount_token: float) -> str:
    ...
    amount_token: Die Tokenmenge als Dezimalwert. Sie wird in die kleinste Einheit (unter Berücksichtigung von TOKEN_DECIMALS) umgerechnet.
    ...
    try:
        ...
        # Berechne die assoziierten Token-Konten
        ...
        # Umrechnung in kleinste Einheit (z. B. bei TOKEN_DECIMALS=6: 1 Token = 1_000_000 Einheiten)
        amount_in_smallest_unit = int(amount_token * (10 ** TOKEN_DECIMALS))
        
        # Erstelle die Transfer-Anweisung für SPL-Token (mit Überprüfung der Dezimalstellen)
