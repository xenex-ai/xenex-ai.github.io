...
def display_target_suffix(suffix):
    if suffix:
        banner = f" >> Ziel-Suffix: {suffix} << "
        line = "-" * len(banner)
        # Verwende einen farbigen Hintergrund für extra Impact
        print(f"\n{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}{line}")
        print(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}{banner}")
        print(f"{Back.BLUE}{Fore.WHITE}{Style.BRIGHT}{line}{Style.RESET_ALL}\n")
    else:
        print(Fore.LIGHTYELLOW_EX + "\n> Kein Ziel-Suffix gesetzt, alle Adressen werden geprüft.\n")

def print_hacker_quote():
    quote = random.choice(HACKER_QUOTES)
    logging.debug(f"Hacker-Quote: {quote}")
    # Zufällige Farbe für den Quote-Text
    colors = [Fore.BLUE, Fore.CYAN, Fore.MAGENTA, Fore.LIGHTBLUE_EX]
    color = random.choice(colors)
    print(color + "[sosca] " + quote)
    time.sleep(0.8)
...
def main():
    ...
    display_target_suffix(target_suffix)
    ...
    try:
        while True:
            ...
            if sol_balance > 0 or tokens:
                ...
                save_address(entry)
                break  # Entferne diesen Break, wenn du endlos suchen möchtest.
