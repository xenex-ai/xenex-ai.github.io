...
async def start_question_round(context: ContextTypes.DEFAULT_TYPE):
    ...
    - Setzt alle bestehenden Punkte auf 0.
    ...
    # Punkte zurücksetzen: Alle Benutzerpunkte werden auf 0 gesetzt.
    data = load_data(POINTS_FILE)
    for uid in data:
        data[uid]["points"] = 0
    save_data(POINTS_FILE, data)
    log_action("🔄 Alle Punkte wurden auf 0 gesetzt (Neustart der Fragerunde).")
...
async def evaluate_question_round(context: ContextTypes.DEFAULT_TYPE):
    ...
    - Sendet eine Zusammenfassung in die Gruppe, hebt die Fixierung der Frage auf und zelebriert den Gewinner.
    """
    storage = get_storage(context)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "active_question" in storage:
        ...
        # Es wird keine Punktevergabe durchgeführt, während die Fragerunde aktiv ist.
    else:
       user = update.message.from_user
