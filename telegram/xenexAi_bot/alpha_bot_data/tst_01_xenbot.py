...
def log_activity(user, action):
    ...
    data.append({"user": user, "action": action})  # Neuen Eintrag hinzufügen
...
async def adm_protocol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for entry in activities[-10:]:
        # Falls entry ein Dictionary mit den Schlüsseln "time", "user" und "action" ist
        time_str = entry.get("time", "Unbekannt")
        user_str = entry.get("user", "Unbekannt")
