...
async def send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, reply_markup=None):
    """Antwortet, egal ob als Nachricht oder über CallbackQuery."""
    if update.message:
...
async def add_points(user_id, username, points):
    ...
    # Optionaler Datei-Upload (z.B. zur Synchronisation)
...
async def show_ranking(context: ContextTypes.DEFAULT_TYPE):
    ...
    Erzeugt eine hübsch formatierte Nachricht mit den Besten.
    """
...
async def auto_question(context: ContextTypes.DEFAULT_TYPE):
    """Startet automatisch ein Frage-Event mit zufällig ausgewählter Frage."""
    global event_active
...
async def auto_stop_event(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not event_data:
        ...
        winner_message = (
            ...
            f"🏆 Herzlichen Glückwunsch {user_info['username']}! Du erhältst {bonus_points} Bonuspunkte!"
        )
...
async def start_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Startet ein Frage-Event (nur Admins)."""
    global event_active
...
async def stop_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Beendet das Frage-Event (nur Admins) und kürt den Gewinner."""
    global event_active
    user = update.message.from_user
    if user.username not in ADMIN_USERS:
        await update.message.reply_text("⛔ Keine Berechtigung!")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Vergibt Punkte für Nachrichten:
      ...
      - 2 Punkte, wenn es sich um eine Antwort handelt.
    """
    user = update.message.from_user
...
async def points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Zeigt dem Nutzer seine aktuellen Punkte an."""
    user = update.message.from_user
    data = load_data(POINTS_FILE)
...
async def claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Bietet dem Nutzer die Option, Punkte einzulösen (mit Inline-Button)."""
    user = update.message.from_user
    data = load_data(POINTS_FILE)
    user_points = data.get(str(user.id), {}).get("points", 0)
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command == "/start":
        ...
        if command == "/start_event":
            await query.message.reply_text("Bitte nutze direkt: /start_event")
        elif command == "/stop_event":
            await query.message.reply_text("Bitte nutze direkt: /stop_event")
...
async def adm_protocol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Zeigt die letzten 100 Aktivitäten (Admin-Protokoll) an."""
    user = update.message.from_user
    if user.username not in ADMIN_USERS:
        await update.message.reply_text("⛔ Keine Berechtigung!")
        return
...
async def lottery(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if outcome == "win":
        ...
        message = f"😕 Schade, {user.username or user.first_name} – kein Gewinn diesmal."
    else:
...
adventure_steps = {
    ...
    "plateau": {
        "text": "Auf dem Sternenplateau ruhst du aus – keine Punkte, aber du fühlst dich erfrischt. 🌠",
        "options": [],
        "end": True
    },
    "light": {
...
async def upgrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Kosten: 200 Punkte.
    Nach 60 Sekunden erhältst du einen zufälligen Bonus zwischen 150 und 500 Punkten.
    """
    user = update.message.from_user
    data = load_data(POINTS_FILE)
    user_id = str(user.id)
    ...
    await update.message.reply_text("🛠️ Dein galaktisches Upgrade wurde gestartet! Bitte warte 60 Sekunden...")
