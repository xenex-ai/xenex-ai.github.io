...
async def add_points(update: Update, context: CallbackContext) -> None:
    ...
    if user_data:
        new_points = user_data[0]["points"] + points
        db.update({"points": new_points}, User.username == username)
    else:
        new_points = points
        db.insert({"username": username, "points": points})

    await update.message.reply_text(f"✅ {points} points added to @{username}! Total: {new_points} points.")
    
    # Send message in the group
    await context.bot.send_message(chat_id=GROUP_ID, text=f"🏆 @{username} received **{points} points**! Total: {new_points} points.")
...
# Function to reward top moderator
async def reward_top_mod(update: Update, context: CallbackContext) -> None:
    leaderboard = sorted(db.all(), key=lambda x: x["points"], reverse=True)
    if not leaderboard:
        await update.message.reply_text("No moderators have points yet!")
        return

    top_mod = leaderboard[0]
    await update.message.reply_text(f"🥇 @{top_mod['username']} has the most points ({top_mod['points']}) and receives a reward!")

# Function to remove points
async def remove_points(update: Update, context: CallbackContext) -> None:
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /removepoints <@username> <points>")
        return
    ...
    if user_data:
        new_points = max(0, user_data[0]["points"] - points)
        db.update({"points": new_points}, User.username == username)
        await update.message.reply_text(f"❌ {points} points removed from @{username}. New balance: {new_points} points.")
        
        # Send message in the group
        await context.bot.send_message(chat_id=GROUP_ID, text=f"⚠️ @{username} lost **{points} points**. New balance: {new_points} points.")
    else:
        await update.message.reply_text(f"@{username} has no points yet.")

# Function to automatically assign points
async def auto_reward_mods(context: CallbackContext) -> None:
    mods = db.all()
    if not mods:
        return
    
    rewarded_mod = random.choice(mods)
    reward_points = random.randint(1, 10) # Random points between 1-10
    new_total = rewarded_mod["points"] + reward_points

    db.update({"points": new_total}, User.username == rewarded_mod["username"])
    
    # Send message in the group
    await context.bot.send_message(chat_id=GROUP_ID, text=f"🎉 @{rewarded_mod['username']} received **{reward_points} points** automatically! Total: {new_total} points.")

# Function to ask questions to the community
...
def main():
    ...
    application.add_handler(CommandHandler("reward", reward_top_mod))

    # Automatically reward moderators every 1 hours
    application.job_queue.run_repeating(auto_reward_mods, interval=7500, first=10)

    # Ask a random question every 1..hours 
