...
GROUP_ID = -100123456789  # Deine Telegram-Gruppen-ID
...
async def welcome_back_inactive_users(context: CallbackContext):
    ...
    now = datetime.now()
    inactive_threshold = now - timedelta(days=7)  # 7 Tage inaktiv
...
async def main():
    ...
    job_queue.run_repeating(welcome_back_inactive_users, interval=86400, first=30)  # Inaktive Nutzer prüfen (täglich)
