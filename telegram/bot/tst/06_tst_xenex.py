...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    command_text = update.message.text.replace("/start ", "").strip()
    ...
    if command_text:
        ...
        if len(parts) == 3 and parts[0] in ["removepoints", "addpoints"]:
            ...
            try:
                ...
                await update.message.reply_text("⚠️ Ungültige Anzahl von Punkten.")
                return
            ...
            await update.message.reply_text(f"🔹 Unbekannter Befehl: {command_text}")
...
async def points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data = next((info for info in data.values() if info.get("username") == user.username), None)
